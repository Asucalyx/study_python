...
"""
    ...
    尾部追加单个元素 .append(追加的元素)方法，追加的元素可以是数据容器
    尾部追加成多个元素 .extend(追加的元素)方法，追加的元素可以是数据容器
    多个列表的连接 + 符号
    统计指定元素数量 .count(统计的元素)方法
    插入指定元素到指定位置 .insert(插入的下标位置,插入的内容)方法
    修改 list[修改位置下标] = 修改内容
    删除指定位置 del list[删除位置下标]
    删除(提取)特定内容 .pop(内容)方法
    删除指定内容 .remove(删除内容)方法
    清空列表 .clear()方法
"""
# 增删改查
